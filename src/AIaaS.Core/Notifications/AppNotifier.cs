...
namespace AIaaS.Notifications
{
    public class AppNotifier : AIaaSDomainServiceBase, IAppNotifier
    {
        ...
        public async Task MyPasswordChanged(User user, string detailedMessage)
        {
            await _notificationPublisher.PublishAsync(
                AppNotificationNames.MyPasswordChanged,
                 new MessageNotificationData(detailedMessage),
                userIds: new[] { user.ToUserIdentifier() }
            );
        }

        public async Task TrainedModelChanged(int? tenantId, string detailedMessage, NotificationSeverity severity)
        {
            await _notificationPublisher.PublishAsync(AppNotificationNames.TrainedModelChanged,
                 new MessageNotificationData(detailedMessage),
                 severity: severity,
