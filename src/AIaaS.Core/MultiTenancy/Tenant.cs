...
namespace AIaaS.MultiTenancy
{
    ...
    public class Tenant : AbpTenant<User>
    {
        ...
        [DisableAuditing]
        public virtual double NlpPriority { get; set; }         //越低等級越高

        [DisableAuditing]
        public virtual double SubscriptionAmount { get; set; }


        [StringLength(256)]
        public virtual string NormalizedRegistrantEmail { get; set; }


        public virtual bool EnableSysGpt { get; set; }
        ...
        public void UpdateSubscriptionDateForPayment(PaymentPeriodType paymentPeriodType, EditionPaymentType editionPaymentType)
        {
            switch (editionPaymentType)
            {
                ...
                case EditionPaymentType.BuyNow:
                    {
                        SubscriptionEndDateUtc = Clock.Now.ToUniversalTime().AddDays((int)paymentPeriodType + 1);
                        break;
                    }
