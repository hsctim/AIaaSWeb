...
namespace AIaaS.Authorization.Roles
{
    /// <summary>
    /// Application service that is used by 'role management' page.
    /// </summary>
    [AbpAuthorize(AppPermissions.Pages_Administration_Roles)]
    public class RoleAppService : AIaaSAppServiceBase, IRoleAppService
    {
        ...
        public async Task<GetRoleForEditOutput> GetRoleForEdit(NullableIdDto input)
        {
            ...
            else
            {
                ...
                string[] permissionsNameArray = {
                    AppPermissions.Pages,
                    AppPermissions.Pages_NlpChatbot,
                    ...
                    AppPermissions.Pages_NlpChatbot_NlpCbQAAccuracies,
                };

                List<string> permissionsName = permissionsNameArray.ToList();

                if (AbpSession.TenantId != null)
                {
                    permissionsName.Add(AppPermissions.Pages_Tenant_Dashboard);
                }


                grantedPermissions = (from p in permissionsName
                                      select new Permission(p)).ToArray();
