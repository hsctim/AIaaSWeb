...
namespace AIaaS.Localization
{
    ...
    public class LanguageAppService : AIaaSAppServiceBase, ILanguageAppService
    {
        ...
        {
            var languages = _applicationLanguageManager.GetActiveLanguages(AbpSession.TenantId).Where(e => e.Name == input.Name);

            if (languages.Any())
                await _applicationLanguageManager.SetDefaultLanguageAsync(AbpSession.TenantId, CultureHelper.GetCultureInfoByChecking(input.Name).Name
            );
            else
            {
                throw new UserFriendlyException(L("ChangeDefaultLanguageError"));
            }
        }
