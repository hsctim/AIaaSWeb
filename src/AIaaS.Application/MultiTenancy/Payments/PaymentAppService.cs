...
{
    public class PaymentAppService : AIaaSAppServiceBase, IPaymentAppService
    {
        ...
        public async Task<PaymentInfoDto> GetPaymentInfo(PaymentInfoInput input)
        {
            ...
            if (additionalPrice > 0)
                additionalPrice = Math.Max(AIaaSConsts.MinimumUpgradePaymentAmount, additionalPrice);
        ...
        public async Task<long> CreatePayment(CreatePaymentDto input)
        {
            ...
            var olderEditions = await _subscriptionPaymentRepository.GetAll().Where(e => e.TenantId == AbpSession.GetTenantId() && e.Status == SubscriptionPaymentStatus.NotPaid).ToListAsync();
            foreach (var olderEdition in olderEditions)
                olderEdition.SetAsCancelled();
        ...
        {
            var description = L(editionPaymentType + "_Edition_Description", L(targetEditionName));
