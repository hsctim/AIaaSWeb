...
namespace ReflectSoftware.Facebook.Messenger.Client
{
    ...
    public class ClientMessenger : IClientMessenger
    {
        ...
        public ClientMessenger(string accessToken, string appSecret, string version = "8.0")
        {
            _accessToken = accessToken;
            _apiVersion = version;
        ...
        protected async Task<object> PostTaskAsync(string path, object parameters)
        {
            ...
            {
                var url = $"https://graph.facebook.com/v{_apiVersion}/{path}?access_token={_accessToken}";

                if (string.IsNullOrEmpty(_appSecret) == false)
                {
                    url += "&appsecret_proof=" + HMACSHA256(_accessToken, _appSecret);
                }
        ...
        protected async Task<TResult> GetTaskAsync<TResult>(object parameters)
        {
            using (var client = new HttpClient())
            {
                try
                {
                    var url = $"https://graph.facebook.com/{parameters}&access_token={_accessToken}";
        ...
        public async Task<UserProfile> GetUserProfileAsync(string userId)
        {
            ...
            return await GetTaskAsync<UserProfile>($"{userId}?fields=name,profile_pic");
        }
        #endregion
