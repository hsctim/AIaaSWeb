...
{
    public class AppMenuViewComponent : AIaaSViewComponent
    {
            ...
            else
            {
                if (AbpSession.TenantId != 1)
                {
                    string[] disabledMenu = { 
                        ...
                        AppPageNames.Common.DynamicEntityProperties,
                        AppPageNames.Common.OrganizationUnits,
                        AppPageNames.Tenant.Settings,
                        //AppPageNames.Tenant.SubscriptionManagement,
                    };

                    foreach (var item in disabledMenu)
                    {
                        var found = FindMenuItemOrNull(model.Menu.Items, item);
                        if (found != null)
